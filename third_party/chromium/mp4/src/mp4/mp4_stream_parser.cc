...
#include "mp4/media_util.h"
#include "mp4/stream_parser_buffer.h"
#include "mp4/text_track_config.h"
#include "mp4/timestamp_constants.h"
#include "mp4/video_decoder_config.h"
#include "mp4/video_util.h"
#include "mp4/box_definitions.h"
#include "mp4/box_reader.h"
#include "mp4/es_descriptor.h"
#include "mp4/rcheck.h"
#include "mp4/adts_constants.h"
...
bool MP4StreamParser::ParseBox(bool* err) {
  ...
  if (reader->type() == FOURCC_MOOV) {
    ...
    *err = !ParseSidx(reader.get(), queue_.tail());
  } else {
...
bool MP4StreamParser::ParseSidx(BoxReader* reader, uint64_t initial_pos) {
  uint64_t offset = initial_pos;
  int32_t full_atom;
  if (!reader->Read4s(&full_atom)) {
    return false;
  }
  int32_t version = 0x000000FF & (full_atom >> 24);
  ...
  if (version == 0) {
    ...
    if (!reader->Read4(&first_offset)) {
      return false;
    }
    earliest_presentation_time = ept;
    offset += first_offset;
  } else {
    if (!reader->Read8(&earliest_presentation_time)) {
      return false;
    }
    uint64_t first_offset;
    if (!reader->Read8(&first_offset)) {
      return false;
    }
    offset += first_offset;
  }
  ...
  // Skip reserved bytes.
  if (!reader->SkipBytes(2)) {
    return false;
  }

  uint16_t reference_count;
  if (!reader->Read2(&reference_count)) {
    return false;
  }

  std::unique_ptr<std::vector<uint32_t>> sizes(new std::vector<uint32_t>);
  std::unique_ptr<std::vector<uint64_t>> offsets(new std::vector<uint64_t>);
  std::unique_ptr<std::vector<uint64_t>> durations_us(
      new std::vector<uint64_t>);
  std::unique_ptr<std::vector<uint64_t>> times_us(new std::vector<uint64_t>);

  uint64_t time = earliest_presentation_time;
  uint64_t time_us = ScaleLargeTimestamp(time, 1000000, timescale);
  for (int i = 0; i < reference_count; i++) {
    uint32_t first_int;
    if (!reader->Read4(&first_int)) {
      return false;
    }
    uint32_t type = 0x80000000 & first_int;
    if (type != 0) {
      return false;
    }

    uint32_t reference_duration;
    if (!reader->Read4(&reference_duration)) {
      return false;
    }

    sizes->push_back(0x7FFFFFFF & first_int);
    offsets->push_back(offset);

    times_us->push_back(time_us);
    time += reference_duration;
    time_us = ScaleLargeTimestamp(time, 1000000, timescale);
    durations_us->push_back(time_us - times_us->at(i));
...
int64_t MP4StreamParser::ScaleLargeTimestamp(int64_t timestamp,
                                             int64_t multiplier,
                                             int64_t divisor) {
  ...
  for (std::vector<Track>::const_iterator track = moov_->tracks.begin();
    ...
    if (track->media.handler.type == kAudio) {
      ...
      if (entry.samplesize == 8) {
        ...
        if (ESDescriptor::IsAAC(audio_type)) {
          sample_format = kSampleFormatPlanarF32;
        } else {
    ...
    if (track->media.handler.type == kVideo) {
      ...
      Size coded_size(entry.width, entry.height);
      Rect visible_rect(coded_size);
      ...
      Size natural_size(visible_rect.size());
      if (entry.pixel_aspect.h_spacing != 1 ||
        ...
        natural_size =
            Size(track->header.width, track->header.height);
      }
